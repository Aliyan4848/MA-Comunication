...
import { CalendarDays, CircleDollarSign, Plus, RefreshCw, Save, Trash2 } from "lucide-react"
...
type Expense = { id: string; name: string; category: string; amount: number; incurred_on: string; notes: string | null; voided_at?: string | null; void_reason?: string | null }
...
export default function AdminProfit() {
  ...
  const load = useCallback(async () => {
    ...
    const [current, annual, month, expenseRows, shareRows] = await Promise.all([
      ...
      supabase.from("business_expenses").select("id,name,category,amount,incurred_on,notes,voided_at,void_reason").gte("incurred_on", range.start).lte("incurred_on", range.end).order("incurred_on", { ascending: false }),
  ...
  const voidExpense = async (expense: Expense) => {
    ...
    if (!window.confirm(`Void “${expense.name}”? It will be excluded from profit totals and retained in the audit history.`)) return
    const reason = window.prompt("Reason for voiding this expense:")
    if (reason === null) return
    if (!reason.trim()) return toast("Enter a reason so the change is auditable.", "error")
    const { error } = await supabase.rpc("admin_void_business_expense", {
    ...
    toast("Expense voided and retained in the audit history.")
    await load()
  }
  ...
  return <div className="space-y-6 text-[var(--ma-foreground)]">
    ...
    <section className="rounded-2xl border border-[var(--ma-border)] bg-[var(--ma-card)] p-4 sm:p-5">
      <div className="mb-4"><h2 className="font-semibold">Business expenses</h2><p className="mt-1 text-xs text-[var(--ma-muted)]">Voided expenses remain in the audit history and are excluded from profit totals.</p></div>
      <form onSubmit={saveExpense} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        <input required placeholder="Expense name" value={expenseForm.name} onChange={e => setExpenseForm({ ...expenseForm, name: e.target.value })} className="rounded-lg border border-[var(--ma-border)] bg-[var(--ma-background)] p-2 text-sm" />
        <select value={expenseForm.category} onChange={e => setExpenseForm({ ...expenseForm, category: e.target.value })} className="rounded-lg border border-[var(--ma-border)] bg-[var(--ma-background)] p-2 text-sm">{categories.map(category => <option key={category}>{category}</option>)}</select>
        <input required type="number" min="0.01" step="0.01" placeholder="Amount (PKR)" value={expenseForm.amount} onChange={e => setExpenseForm({ ...expenseForm, amount: e.target.value })} className="rounded-lg border border-[var(--ma-border)] bg-[var(--ma-background)] p-2 text-sm" />
        <input required type="date" value={expenseForm.expense_date} onChange={e => setExpenseForm({ ...expenseForm, expense_date: e.target.value })} className="rounded-lg border border-[var(--ma-border)] bg-[var(--ma-background)] p-2 text-sm" />
        <input placeholder="Notes" value={expenseForm.notes} onChange={e => setExpenseForm({ ...expenseForm, notes: e.target.value })} className="rounded-lg border border-[var(--ma-border)] bg-[var(--ma-background)] p-2 text-sm" />
        <button className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#2B8EF0] px-3 py-2 text-sm font-semibold text-white"><Plus size={15} />{editing ? "Update" : "Add expense"}</button>
        {editing && <button type="button" onClick={() => { setEditing(null); setExpenseForm({ name: "", category: "Other", amount: "", expense_date: dateString(new Date()), notes: "" }) }} className="rounded-lg border border-[var(--ma-border)] px-3 py-2 text-sm">Cancel edit</button>}
      </form>
      <div className="mt-4 divide-y divide-[var(--ma-border)]">
        {expenses.map(expense => <div key={expense.id} className={expense.voided_at ? "flex flex-wrap items-center justify-between gap-2 py-3 text-sm opacity-60" : "flex flex-wrap items-center justify-between gap-2 py-3 text-sm"}>
          <div><b>{expense.name}</b><span className="ml-2 text-[var(--ma-muted)]">{expense.category} · {expense.incurred_on}</span>{expense.voided_at && <span className="ml-2 rounded bg-red-500/10 px-2 py-1 text-xs text-red-400">Voided</span>}{expense.notes && <p className="mt-1 text-xs text-[var(--ma-muted)]">{expense.notes}</p>}{expense.void_reason && <p className="mt-1 text-xs text-red-400">Void reason: {expense.void_reason}</p>}</div>
          <div className="flex items-center gap-3"><b>{money(expense.amount)}</b>{!expense.voided_at && <><button type="button" onClick={() => { setEditing(expense.id); setExpenseForm({ name: expense.name, category: expense.category, amount: String(expense.amount), expense_date: expense.incurred_on, notes: expense.notes || "" }) }} className="text-[#2B8EF0]">Edit</button><button type="button" onClick={() => voidExpense(expense)} aria-label="Void expense" title="Void expense" className="inline-flex items-center gap-1 text-red-400"><Trash2 size={15} />Void</button></>}</div>
        </div>)}
        {!expenses.length && <p className="py-4 text-sm text-[var(--ma-muted)]">No expenses recorded for this period.</p>}
      </div>
    </section>
