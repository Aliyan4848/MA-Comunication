...
export default function OrderConfirmation() {
  ...
  useEffect(() => {
    ...
    fetchPublicOrder(id)
      ...
      .catch(() => { if (mounted) { setOrder(null); setFetchFailed(true) } })
    return () => { mounted = false }
  }, [id, fetchPublicOrder])

  // Genuinely nothing to show yet: still loading AND we didn't arrive here
  // via a successful checkout (no state), so there's no fallback data either.
  if (order === undefined && !checkoutState.orderNumber) {
    return (
  ...
  // Truly nothing to show: the fetch resolved to "not found" AND we have no
  // checkout state to fall back on. This is the only case that should ever
  // show a dead end — e.g. someone opened a stale/invalid link directly with
  // no prior checkout in this browser session.
  if (order === null && !checkoutState.orderNumber) {
    return (
      <div className="pt-28 min-h-screen flex items-center justify-center px-4">
  ...
  const orderNumber = order?.orderNumber || checkoutState.orderNumber || ""
  const total = order?.total ?? checkoutState.total

  // We have at least the order number (from checkout state), but the full
  // detail fetch either hasn't resolved yet or failed. Show a real, useful
  // confirmation instead of a loading spinner or a dead end — this is
  // exactly what the customer needs immediately after placing an order,
  // and it degrades gracefully regardless of why the second fetch is slow
  // or unavailable (network hiccup, guest session quirk, etc.).
  if (!order) {
    return (
        ...
        <div className="w-full max-w-lg">
          ...
          >
            <div className="px-6 py-4 border-b border-[var(--ma-border)] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-[var(--ma-foreground)]">Order Confirmed</span>
              </div>
              <span className="text-xs font-mono text-[#2B8EF0] bg-[#2B8EF0]/10 px-2 py-1 rounded-lg">{orderNumber}</span>
            </div>
            <div className="p-6 space-y-3">
              ...
              <div className="bg-[var(--ma-surface)] rounded-xl px-4 py-3 text-xs text-[var(--ma-muted)]">
                Payment: <span className="text-emerald-400 font-medium">Cash on Delivery</span>
              </div>
              <p className="text-xs text-[var(--ma-muted)] pt-1">
                {fetchFailed || order === null
                  ? "We couldn't load the full order details right now, but your order was placed successfully — save your order number above."
          ...
          <div className="flex gap-3">
            <Link
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#2B8EF0] hover:bg-[#1A7DE0] text-white rounded-xl transition-all text-sm font-medium"
            >
              <Search size={14} /> Track Order
            </Link>
            <Link
              to="/shop"
              className="flex-1 flex items-center justify-center gap-2 py-3 border border-[var(--ma-border)] text-[var(--ma-muted)] hover:text-[var(--ma-foreground)] hover:border-[var(--ma-border-hover)] rounded-xl transition-all text-sm font-medium"
            >
          ...
          {!session && (
            ...
            >
              ...
              <Link
                to="/signup"
                className="inline-block mt-2 px-5 py-2 bg-[#2B8EF0] hover:bg-[#1A7DE0] text-white text-sm font-medium rounded-xl transition-all"
              >
                Create Account
              </Link>
            </motion.div>
  ...
  // Full success path: the detailed fetch came back, show everything.
  return (
    <div className="pt-20 min-h-screen flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-lg">
        <motion.div
